    ...
    var body: some View {
        ZStack(alignment: .bottomLeading){
            if let url = URL(string: urlString) {
                ...
                NoirImage(url: url) { ProgressView() }
                    .resizable()
                    .aspectRatio(0.72, contentMode: .fill)
                    .onAppear { print("Image appeared \(urlString)") }
                ...
                KFImage(url)
                    .onSuccess { result in
                        ...
                        case .none: print("Image loaded from network.")
                        case .memory: print("Image loaded from memory cache.")
                        case .disk: print("Image loaded from disk cache.")
                        @unknown default: print("Unknown cache type.")
                        }
                    }
