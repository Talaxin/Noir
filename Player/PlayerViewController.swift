...
final class PlayerViewController: UIViewController, UIGestureRecognizerDelegate, UIAdaptivePresentationControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        setupHoldGesture()
#if !os(tvOS)
        setupVolumeBrightnessPanGesture()
        setupVolumeSlider()
#endif
    ...
    private func setupLayout() {
        ...
        videoContainer.addSubview(touchOverlay)
        volumeBrightnessTouchOverlay = touchOverlay
        NSLayoutConstraint.activate([
            touchOverlay.topAnchor.constraint(equalTo: videoContainer.topAnchor),
            touchOverlay.leadingAnchor.constraint(equalTo: videoContainer.leadingAnchor),
            touchOverlay.trailingAnchor.constraint(equalTo: videoContainer.trailingAnchor),
            touchOverlay.bottomAnchor.constraint(equalTo: videoContainer.bottomAnchor)
        ])
#endif
    ...
    private func setupVolumeBrightnessOverlays() {
        ...
        NSLayoutConstraint.activate([
            brightnessContainer.leadingAnchor.constraint(equalTo: videoContainer.safeAreaLayoutGuide.leadingAnchor, constant: 24),
            ...
            volumeContainer.trailingAnchor.constraint(equalTo: videoContainer.safeAreaLayoutGuide.trailingAnchor, constant: -24),
            volumeContainer.centerYAnchor.constraint(equalTo: videoContainer.centerYAnchor),
    ...
    private func setupVolumeBrightnessPanGesture() {
        guard let overlay = volumeBrightnessTouchOverlay else { return }
    ...
    @objc private func handleVolumeBrightnessPan(_ gesture: UIPanGestureRecognizer) {
        ...
        case .changed:
            ...
            gesture.setTranslation(.zero, in: videoContainer)
...
extension PlayerViewController {
    private static let sideZoneFraction: CGFloat = 0.35
    
    func gestureRecognizerShouldBegin(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
        let loc = gestureRecognizer.location(in: videoContainer)
        let w = videoContainer.bounds.width
        let leftEdge = w * Self.sideZoneFraction
        let rightEdge = w * (1 - Self.sideZoneFraction)
        let isInLeftZone = loc.x < leftEdge
        let isInRightZone = loc.x > rightEdge
        let isInSideZone = isInLeftZone || isInRightZone
        
        var result = true
        if gestureRecognizer === holdGesture {
            result = !isInSideZone
        } else if gestureRecognizer === volumeBrightnessPanGesture {
            result = isInSideZone
        } else if gestureRecognizer === containerTapGesture {
            result = !isInSideZone
        }
        return result
    }
    ...
    func gestureRecognizer(_ gestureRecognizer: UIGestureRecognizer, shouldRecognizeSimultaneouslyWith otherGestureRecognizer: UIGestureRecognizer) -> Bool {
        guard gestureRecognizer === volumeBrightnessPanGesture else { return false }
        return true
    }
    ...
    func renderer(_ renderer: MPVSoftwareRenderer, didBecomeReadyToSeek: Bool) {
        DispatchQueue.main.async { [weak self] in
            ...
            if let seekTime = self.pendingSeekTime {
                let duration = self.cachedDuration
                let clampedSeek: Double
                if duration.isFinite, duration > 1 {
                    clampedSeek = min(max(0, seekTime), max(0, duration - 1.0))
                } else {
                    clampedSeek = max(0, seekTime)
                }
                if clampedSeek > 0 {
                    self.renderer.seek(to: clampedSeek)
                    Logger.shared.log("Resumed MPV playback from \(Int(clampedSeek))s (requested \(Int(seekTime))s)", type: "Progress")
                } else {
                    Logger.shared.log("Skipped MPV resume seek (requested=\(Int(seekTime))s, duration=\(Int(duration))s)", type: "Progress")
                }
