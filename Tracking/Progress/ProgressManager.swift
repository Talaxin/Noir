...
final class ProgressManager {
    ...
    private let accessQueue = DispatchQueue(label: "com.maxtori.noir.progress-manager", attributes: .concurrent)
    
    /// When opening a download in an external player (e.g. Infuse), we store its media info so the x-callback-url can mark it watched.
    private static var pendingExternalPlaybackMediaInfo: MediaInfo?
    ...
    func replaceProgressDataForRestore(_ newData: ProgressData) {
        accessQueue.async(flags: .barrier) { [weak self] in
            ...
            Logger.shared.log("Progress data restored (\(newData.movieProgress.count) movies, \(newData.episodeProgress.count) episodes)", type: "Progress")
        }
        saveProgressData()
    }
    
    func bulkMarkEpisodesAsWatched(showId: Int, seasonNumber: Int, throughEpisodeNumber: Int) {
        for ep in 1...throughEpisodeNumber {
            markEpisodeAsWatched(showId: showId, seasonNumber: seasonNumber, episodeNumber: ep)
        }
    }
    ...
    func markEpisodeAsWatched(showId: Int, seasonNumber: Int, episodeNumber: Int) {
        accessQueue.async(flags: .barrier) { [weak self] in
            ...
            var entry = self.progressData.findEpisode(showId: showId, season: seasonNumber, episode: episodeNumber)
                ?? EpisodeProgressEntry(showId: showId, seasonNumber: seasonNumber, episodeNumber: episodeNumber)
            entry.isWatched = true
            entry.currentTime = max(entry.currentTime, entry.totalDuration > 0 ? entry.totalDuration : 1)
            entry.totalDuration = max(entry.totalDuration, entry.currentTime)
            entry.lastUpdated = Date()
            self.progressData.updateEpisode(entry)
    ...
    /// Call before opening a download in an external player (e.g. Infuse) so we can mark it watched when the app returns via x-callback-url.
    func setPendingExternalPlayback(_ info: MediaInfo?) {
        Self.pendingExternalPlaybackMediaInfo = info
    }
    
    /// Called when the app receives x-callback-url/playbackDidFinish (e.g. from Infuse). If we had pending media from a download, mark it watched and clear.
    func takePendingExternalPlaybackAndMarkWatched() {
        guard let info = Self.pendingExternalPlaybackMediaInfo else { return }
        Self.pendingExternalPlaybackMediaInfo = nil
        switch info {
        case .movie(let id, let title):
            markMovieAsWatched(movieId: id, title: title)
        case .episode(let showId, _, let seasonNumber, let episodeNumber):
            markEpisodeAsWatched(showId: showId, seasonNumber: seasonNumber, episodeNumber: episodeNumber)
        }
    }
    
    func markEpisodeAsUnwatched(showId: Int, seasonNumber: Int, episodeNumber: Int) {
        resetEpisodeProgress(showId: showId, seasonNumber: seasonNumber, episodeNumber: episodeNumber)
    }
