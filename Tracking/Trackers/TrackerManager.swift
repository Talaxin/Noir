...
final class TrackerManager: NSObject, ObservableObject {
    ...
    private let anilistIdCacheQueue = DispatchQueue(label: "com.maxtori.noir.anilistIdCache")
    ...
    private let anilistSeasonIdCacheQueue = DispatchQueue(label: "com.maxtori.noir.anilistSeasonIdCache")

    // Prevent tracker sync bursts during local backup restore.
    private var syncSuppressedDuringBackupRestore = false
    private let backupRestoreSyncQueue = DispatchQueue(label: "com.maxtori.noir.backupRestoreSync")
    ...
    private var anilistRedirectUri: String {
        Bundle.main.object(forInfoDictionaryKey: "AniListRedirectUri") as? String ?? "noir://anilist-callback"
    }
    ...
    private var traktRedirectUri: String {
        Bundle.main.object(forInfoDictionaryKey: "TraktRedirectUri") as? String ?? "noir://trakt-callback"
    }
    ...
    func startAniListAuth() {
        ...
        #else
        let session = ASWebAuthenticationSession(url: url, callbackURLScheme: "noir") { [weak self] callbackURL, error in
                        ...
                        for entry in importEntries {
                            ...
                            if entry.episodesWatched > 0 {
                                ProgressManager.shared.bulkMarkEpisodesAsWatched(
                                    ...
                                    seasonNumber: 1,
                                    throughEpisodeNumber: entry.episodesWatched
                                )
                            }
                        }
