...
/// Wrapper so we can use fullScreenCover(item:) with TMDBSearchResult for download flow (file-local for MediaSection/MediaCard).
struct DownloadableMediaItem: Identifiable {
...
struct HomeView: View {
    ...
    @State private var downloadTarget: DownloadableMediaItem?
    ...
    private var homeContent: some View {
        ...
        .modifier(HiddenToolbarBackgroundIfAvailable())
#endif
        .onAppear {
            if !hasLoadedContent {
                loadContent()
        ...
        if let hero = heroContent {
            VStack(alignment: .center, spacing: isTvOS ? 30 : 12) {
                ...
                if let heroLogoURL = heroLogoURL {
                    NoirImage(url: URL(string: heroLogoURL)) { heroTitleText(hero) }
                        .resizable()
    ...
    private func loadContent() {
        ...
        Task {
            do {
                // Sequential loading to avoid "freed pointer was not the last allocation" (SIGABRT) on device and simulator.
                let trendingResult = try await tmdbService.getTrending()
                let popularMoviesResult = try await tmdbService.getPopularMovies()
                let popularTVResult = try await tmdbService.getPopularTVShows()
                let popularAnimeResult = try await tmdbService.getPopularAnime()
                let topRatedMoviesResult = try await tmdbService.getTopRatedMovies()
                let topRatedTVResult = try await tmdbService.getTopRatedTVShows()
                let topRatedAnimeResult = try await tmdbService.getTopRatedAnime()
                await MainActor.run {
...
struct MediaSection: View {
    ...
    let isLarge: Bool
    @Binding var downloadTarget: DownloadableMediaItem?
    
    var gap: Double { isTvOS ? 50.0 : 20.0 }
    
    init(title: String, items: [TMDBSearchResult], downloadTarget: Binding<DownloadableMediaItem?>, isLarge: Bool = Bool.random()) {
        self.title = title
        self.items = items
        self._downloadTarget = downloadTarget
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                LazyHStack(spacing: gap) {
                    ForEach(items) { item in
                        MediaCard(result: item, downloadTarget: $downloadTarget)
                    }
                }
                .padding(.horizontal, isTvOS ? 40 : 16)
            }
    ...
    var body: some View {
        NavigationLink(destination: MediaDetailView(searchResult: result)) {
            VStack(alignment: .leading, spacing: 6) {
                NoirImage(url: URL(string: result.fullPosterURL ?? "")) {
                        FallbackImageView(
        ...
        .contextMenu {
            Button {
                downloadTarget = DownloadableMediaItem(result: result)
            } label: {
            ...
            ZStack(alignment: .bottomLeading) {
                ZStack {
                    if let backdropURL = backdropURL {
                        NoirImage(url: URL(string: backdropURL)) { backdropPlaceholder }
                            .resizable()
                ...
                VStack(alignment: .leading, spacing: isTvOS ? 10 : 6) {
                    ...
                    HStack(alignment: .bottom, spacing: isTvOS ? 12 : 8) {
                        if let logoURL = logoURL {
                            NoirImage(url: URL(string: logoURL)) { titleText }
                                .resizable()
                                .aspectRatio(contentMode: .fit)
...
#if os(iOS)
private struct HiddenToolbarBackgroundIfAvailable: ViewModifier {
    func body(content: Content) -> some View {
        if #available(iOS 16.0, *) {
