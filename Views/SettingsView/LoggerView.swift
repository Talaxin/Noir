...
struct LoggerView: View {
    ...
    @State private var isCopyingLogs = false
    ...
    var body: some View {
        ...
        .toolbar {
            ToolbarItemGroup(placement: .navigationBarTrailing) {
                Button {
#if !os(tvOS)
                    guard !isCopyingLogs else { return }
                    isCopyingLogs = true
                    Task {
                        let logs = await Logger.shared.getLogsAsync()
                        UIPasteboard.general.string = logs
                        Logger.shared.log("Copied logs to clipboard (\(logs.count) chars)", type: "General")
                        await MainActor.run { isCopyingLogs = false }
                    }
#endif
                } label: {
                    if isCopyingLogs {
                ...
                .accessibilityLabel(isCopyingLogs ? "Copying Logs" : "Copy Logs")
