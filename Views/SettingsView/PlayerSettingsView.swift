...
enum ExternalPlayer: String, CaseIterable, Identifiable {
    ...
    func schemeURL(for urlString: String) -> URL? {
        // Infuse (and other x-callback players) require the url param to be percent-encoded per Firecore docs; otherwise ? & = in the stream URL break the query string.
        let querySafe = CharacterSet.alphanumerics.union(CharacterSet(charactersIn: "-_.~"))
        let encodedForQuery = urlString.addingPercentEncoding(withAllowedCharacters: querySafe) ?? urlString
        // nPlayer expects "nplayer-https://..." with : and / not encoded.
        let nPlayerURL = urlString.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? urlString
        switch self {
        case .infuse:
            // x-success so Infuse can return to Noir when playback ends; may reduce temp "Syncing with iCloud" item
            let callback = "sora://x-callback-url/playbackDidFinish".addingPercentEncoding(withAllowedCharacters: querySafe) ?? "sora://x-callback-url/playbackDidFinish"
            return URL(string: "infuse://x-callback-url/play?x-success=\(callback)&url=\(encodedForQuery)")
        case .vlc:
            return URL(string: "vlc://\(encodedForQuery)")
        case .outPlayer:
            return URL(string: "outplayer://\(encodedForQuery)")
        case .nPlayer:
            return URL(string: "nplayer-\(nPlayerURL)")
        case .senPlayer:
            return URL(string: "senplayer://x-callback-url/play?url=\(encodedForQuery)")
        case .tracy:
            return URL(string: "tracy://open?url=\(encodedForQuery)")
        case .vidHub:
            return URL(string: "open-vidhub://x-callback-url/open?url=\(encodedForQuery)")
        case .none:
            return nil
        }
    }
...
struct PlayerSettingsView: View {
    ...
    var body: some View {
        List {
            ...
            Section(header: Text("Media Player"), footer: Text("If Infuse shows a temporary \"Syncing with iCloud\" item after playing, you can hide it by turning off Links in Infuse Settings > Library.")) {
                HStack {
                    VStack(alignment: .leading, spacing: 2) {
                        Text("Media Player")
                            .font(.subheadline)
