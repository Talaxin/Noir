...
struct StorageView: View {
    ...
    var body: some View {
        List {
            Section(header: Text("DOWNLOAD LOCATION"), footer: Text("Choose a folder in Files (e.g. On My iPhone or iCloud Drive) so other apps like Infuse can see your downloads. New downloads will go there; existing ones stay in Noir’s folder.")) {
                HStack {
                    Text("Location")
                    Spacer()
                    Text(downloadManager.usesCustomDownloadsDirectory ? (downloadManager.customDownloadsDirectoryDisplayName ?? "Custom") : "Noir (private)")
                ...
                .disabled(downloadManager.activeDownloadCount > 0)
                if downloadManager.usesCustomDownloadsDirectory {
                    Button("Use Noir folder", role: .destructive) {
                        downloadManager.clearCustomDownloadsDirectory()
                    }
                    .disabled(downloadManager.activeDownloadCount > 0)
                }
            }
        ...
        .sheet(isPresented: $showFolderPicker) {
            FolderPickerView(
                onPick: { url in
                    downloadManager.setCustomDownloadsDirectory(url)
