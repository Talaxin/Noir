...
struct DownloadsView: View {
                ...
                List {
                    ForEach(groupedDownloads) { show in
                        if show.isMovie {
                            ...
                            .contextMenu {
#if os(iOS)
                                let allEpisodes = show.seasons.flatMap { $0.episodes }
                                if allEpisodes.count > 1 {
                                    Button(action: { shareDownloadedItems(allEpisodes) }) {
                                        Label("Share All Episodes (\(allEpisodes.count))", systemImage: "square.and.arrow.up")
                                    }
                                }
#endif
                                Button(role: .destructive) {
    ...
    /// Share one or more downloaded files (e.g. all episodes of a show). Uses temp copies so receiving apps (e.g. Infuse) can open the files.
    ...
    private func openInExternalOrShare(_ item: DownloadItem) {
        ...
        if external != .none {
            // HLS: give Infuse the stream via proxy so it gets proper HLS (local .ts causes "Failed to open input stream in demuxing").
            ...
            // Non-HLS or proxy failed: use local file URL.
    ...
    private func playDownloadedItem(_ item: DownloadItem) {
        ...
        if inAppRaw == "mpv" || inAppRaw == "VLC" {
            ...
            if item.isHLS, let streamURL = URL(string: item.streamURL) {
                ...
                // For non-HLS local files, keep using AVURLAsset with a small TS hint if needed.
