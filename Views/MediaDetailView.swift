...
struct MediaDetailView: View {
    ...
    @State private var searchForDownload = false
    @State private var openDownloadOnAppear = false
    /// When non-nil, we're in "Download Season" flow; sheet will re-open for each episode.
    @State private var seasonDownloadQueue: [TMDBEpisode]?
    @State private var seasonDownloadIndex: Int = 0
    ...
    init(searchResult: TMDBSearchResult, openDownloadOnAppear: Bool = false) {
        self.searchResult = searchResult
        self.moduleContext = nil
        self._openDownloadOnAppear = State(initialValue: openDownloadOnAppear)
    }

    init(moduleItem: SearchItem, service: Service, openDownloadOnAppear: Bool = false) {
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            if openDownloadOnAppear {
                openDownloadOnAppear = false
                searchForDownload = true
                searchInServices()
            }
        }
        ...
        .sheet(isPresented: $showingSearchResults) {
            ModulesSearchResultsSheet(
                ...
                posterURL: searchResult.fullPosterURL,
                downloadIntent: searchForDownload,
                seasonDownloadEpisodeIndex: seasonDownloadQueue.map { _ in seasonDownloadIndex },
                seasonDownloadTotal: seasonDownloadQueue?.count
            )
            .onDisappear {
                if seasonDownloadQueue != nil {
                    seasonDownloadIndex += 1
                    if seasonDownloadIndex < (seasonDownloadQueue?.count ?? 0),
                       let next = seasonDownloadQueue?[seasonDownloadIndex] {
                        ...
                        seasonDownloadQueue = nil
                        seasonDownloadIndex = 0
    ...
    private var downloadButton: some View {
        let canDownloadSeason = !searchResult.isMovie && seasonDetail.map { !$0.episodes.isEmpty } ?? false
        if canDownloadSeason, let episodes = seasonDetail?.episodes, !episodes.isEmpty {
            Menu {
                Button {
                    seasonDownloadQueue = nil
                    searchForDownload = true
                ...
                Button {
                    seasonDownloadQueue = episodes
                    seasonDownloadIndex = 0
                    selectedEpisodeForSearch = episodes[0]
                    searchForDownload = true
                    showingSearchResults = true
                } label: {
                    Label("Download Season (\(episodes.count) episodes)", systemImage: "arrow.down.circle.fill")
                }
            } label: {
