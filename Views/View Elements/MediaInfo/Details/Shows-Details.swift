...
struct TVShowSeasonsSection: View {
    ...
    @State private var downloadIntentForEpisode = false
    @State private var progressRefreshId = 0
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showingSearchResults) {
            ModulesSearchResultsSheet(
                ...
                downloadIntent: downloadIntentForEpisode
            )
            .onDisappear { downloadIntentForEpisode = false }
        }
                    ...
                    HStack(spacing: 12) {
                        ForEach(seasons) { season in
                            Button(action: {
                                ...
                                VStack(spacing: 8) {
                                    NoirImage(url: URL(string: season.fullPosterURL ?? "")) {
                                            Rectangle()
    ...
    private func createEpisodeCell(episode: TMDBEpisode, index: Int) -> some View {
        if let tvShow = tvShow {
            ...
            EpisodeCell(
                ...
                showId: tvShow.id,
                progress: progress,
                progressRefreshId: progressRefreshId,
                ...
                onProgressRefresh: { progressRefreshId += 1 }
            )
        } else {
            EmptyView()
        }
        ...
        Task {
            do {
                ...
                await MainActor.run {
                    ...
                    // If nothing has been selected yet, choose the best episode in this season
                    // to either continue watching or start from.
                    if selectedEpisodeForSearch == nil,
                       let tvShow = tvShow {
                        self.selectedEpisodeForSearch = pickBestEpisodeToResumeOrStart(
                            showId: tvShow.id,
    ...
    /// Determines the most appropriate episode to play for a given season.
    /// Preference order:
    /// 1. An in-progress episode in this season (partially watched), with the highest progress.
    /// 2. The first completely unwatched episode in this season.
    /// 3. The first episode in the season.
    private func pickBestEpisodeToResumeOrStart(showId: Int, episodes: [TMDBEpisode]) -> TMDBEpisode? {
        guard !episodes.isEmpty else { return nil }
        
        let orderedEpisodes = episodes.sorted { $0.episodeNumber < $1.episodeNumber }
        
        var bestInProgress: (episode: TMDBEpisode, progress: Double)?
        var firstUnwatched: TMDBEpisode?
        ...
        for episode in orderedEpisodes {
            ...
            if progress > 0.02 && progress < 0.95 {
                if let currentBest = bestInProgress {
                    if progress > currentBest.progress {
                        bestInProgress = (episode, progress)
                    }
                } else {
                    bestInProgress = (episode, progress)
                }
            } else if progress == 0, firstUnwatched == nil {
                firstUnwatched = episode
            }
        }
        
        if let best = bestInProgress {
            return best.episode
        }
