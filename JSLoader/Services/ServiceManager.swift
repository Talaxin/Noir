...
final class ServiceManager: ObservableObject {
    ...
    private func downloadAndParseMetadata(from urlString: String) async throws -> ServiceMetadata {
        ...
        if data.isEmpty {
            Logger.shared.log("Service manifest: empty response from \(urlString)", type: "ServiceManager")
            throw ServiceError.invalidMetadataFormat
        }
        let prefix = String(data: data.prefix(80), encoding: .utf8) ?? ""
        if prefix.trimmingCharacters(in: .whitespaces).hasPrefix("<") {
            Logger.shared.log("Service manifest looks like HTML, not JSON. Use the raw URL: raw.githubusercontent.com/.../file.json (not github.com/.../blob/...)", type: "ServiceManager")
            throw ServiceError.invalidMetadataFormat
        }
        do {
            return try JSONDecoder().decode(ServiceMetadata.self, from: data)
        } catch {
            Logger.shared.log("Service manifest decode failed: \(error). URL: \(urlString)", type: "ServiceManager")
            throw ServiceError.invalidMetadataFormat
        }
    }
...
enum ServiceError: LocalizedError {
    case invalidURL, invalidScriptURL, downloadFailed, scriptDownloadFailed, invalidJSON, invalidScriptContent, invalidMetadataFormat
    ...
    var errorDescription: String? {
        ...
        case .invalidMetadataFormat:
            return "Not valid service JSON. Paste the raw .json link (raw.githubusercontent.com/.../miruro.json), not the GitHub repo page."
        }
    }
}
