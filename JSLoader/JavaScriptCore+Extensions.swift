...
extension JSContext {
                ...
                return new Promise(function(resolve, reject) {
                    fetchV2Native(url, processedHeaders, method, processedBody, redirect, finalEncoding, function(rawText) {
                        if (rawText && rawText.error) {
                            reject(rawText.error);
                            return;
                        }
                        const responseObj = {
                            headers: rawText.headers || {},
                            status: rawText.status ?? 0,
                            _data: rawText.body != null ? rawText.body : "",
                            text: function() {
    ...
    /// Decodes Miruro-style pipe response: base64(gzip(json)). Returns decompressed JSON string or nil.
    func setupPipeResponseDecoder() {
        let decodeBlock: @convention(block) (String) -> String? = { base64String in
            // Miruro often uses base64url ( '-' and '_' ), sometimes without padding.
            // Normalize to standard base64 and pad to multiple of 4.
            var cleaned = base64String.replacingOccurrences(of: "\\s", with: "", options: .regularExpression)
            cleaned = cleaned.replacingOccurrences(of: "-", with: "+")
            cleaned = cleaned.replacingOccurrences(of: "_", with: "/")
            let pad = cleaned.count % 4
            if pad != 0 {
                cleaned += String(repeating: "=", count: 4 - pad)
            }

            guard let data = Data(base64Encoded: cleaned),
                  data.count > 18 else { return nil }

            // Robust gunzip via zlib (handles all gzip header variants).
            // windowBits = 15 + 32 enables auto zlib/gzip decoding.
            return data.withUnsafeBytes { (srcPtr: UnsafeRawBufferPointer) -> String? in
                guard let srcBase = srcPtr.bindMemory(to: Bytef.self).baseAddress else { return nil }
                ...
                var out = Data()
                out.reserveCapacity(min(1_048_576, srcPtr.count * 4))

                let chunkSize = 64 * 1024
                var buffer = [UInt8](repeating: 0, count: chunkSize)

                while true {
                    buffer.withUnsafeMutableBytes { (dstPtr: UnsafeMutableRawBufferPointer) in
        ...
        self.setObject(decodeBlock, forKeyedSubscript: "decodePipeResponse" as NSString)
    }
    ...
    func setupSoraFetch() {
        ...
        async function soraFetch(url, options) {
            ...
            try {
                ...
                try {
                    var legacy = await fetch(url, headers);
                    return {
                        status: 200,
                        headers: {},
                        text: function() { return Promise.resolve(legacy != null ? String(legacy) : ""); },
                        json: function() {
                            var t = legacy != null ? String(legacy) : "";
