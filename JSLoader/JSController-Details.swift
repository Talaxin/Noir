...
extension JSController {
    private static let episodeFetchTimeoutSeconds: TimeInterval = 45

    private func normalizedModuleURL(_ raw: String) -> String {
        let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
        if let parsed = URL(string: trimmed), parsed.scheme != nil {
            return trimmed
        }
        let path = trimmed.hasPrefix("/") ? String(trimmed.dropFirst()) : trimmed
        return "noir-module:///\(path)"
    }

    func fetchDetailsJS(url: String, completion: @escaping ([MediaItem], [EpisodeLink]) -> Void) {
        let moduleURL = normalizedModuleURL(url)
        guard let url = URL(string: moduleURL) else {
            Logger.shared.log("Invalid URL in fetchDetailsJS: \(moduleURL)", type: "Error")
            completion([], [])
            return
        }
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + Self.episodeFetchTimeoutSeconds, execute: timeoutWorkItem)
    ...
    func fetchEpisodesJS(url: String, completion: @escaping ([EpisodeLink]) -> Void) {
        let moduleURL = normalizedModuleURL(url)
        guard let url = URL(string: moduleURL) else {
            Logger.shared.log("Invalid URL in fetchEpisodesJS: \(moduleURL)", type: "Error")
            completion([])
            return
        }
